<template>
    <div>
        ...
        <div class='search-box'>
            <van-search v-model="searchValue" shape='round' placeholder="请输入歌曲、歌手、专辑" @search='handleToSearch'
                @clear='clearSearchValue' @focus='searchFocus' @input='upadteValue' />
        </div>
        <div v-if='searchStep==1'>
            ...
            <div class="hot-search">
                ...
                <div class='hot-search-content'>
                    <van-button round v-for='(item,index) in this.hotSearchList' :key='index'
                ...
                <div class="search-history">
                    <div class="history-item" v-for="(item,index) in searchHistory" :key='index'>
                        ...
                        <div>
                            ...
                            <van-icon name="cross" @click='handleToDeleteHistory(index)' />
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div v-if='searchStep==2'>
                        ...
                        <div class="idx">
                            {{(index+1) < 10 ? '0' +(index+1) : (index+1)}} </div>
                                <div class='song-info'>
                                    <p>{{item.name}}</p>
                                    <p>
                                        <span v-for='(item,index) in item.artists'>
                                            {{item.name+" "}}
                                        </span>
            ...
            <div v-if='searchStep==3'>
                <div class='search-history'>
                    <div class="history-item" v-for='(item,index) in keyWord' :key='index'>
                        <van-icon name="search" class='clock' />
                        <div>
                            <div @click='handleToSelect(item.keyword)'>{{item.keyword}}</div>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            upadteValue() {
                // console.log(1)
                axios.get('/search/suggest?keywords= ' + this.searchValue + '&type=mobile').then(res => {
                    // console.log(res)
