<template>
    <div>
        ...
        <div class='most' :style="{'backgroundImage': 'url('+this.songsInfo.al.picUrl+')'}"></div>
        <div class="detail-head">
            ...
            <div class="title-name">
                ...
                <p>
                    <span v-for='(item,index) in this.songsInfo.ar'>
                        {{item.name+" "}}
                    </span>
                </p>
            </div>
        ...
        <div class="container">
            ...
            <div :class="{bottom:true,activeRote:this.playFlag}" @click='trogglePlay'>
                <div class="center">
            ...
            <div class="lyric">
                <div class="lyric-wrap" :style='{"margin-top":this.marginTop}'>
                    <p v-for='(item,index) in lyric' :key='index' :class="{activelyric: index===currentIndex}">
                        {{item.content}}
                    </p>
                </div>
            ...
            <div>
                ...
                <div class='play-control'>
                    ...
                    <van-icon v-if='playFlag' name="pause-circle-o" @click='trogglePlay' />
                    <van-icon v-else='!playFlag' name="play-circle-o" @click='trogglePlay' />
        ...
        <div class="bottom-container">
            ...
            <div class="like-songs">
                <div class='like-song' v-for='(item,index) in this.sameSong' :key='index'>
                    <div class="pic">
                        <img :src="item.album.picUrl" alt="">
                    </div>
                    <div class='song-info'>
                        <p>{{item.name}}</p>
                        <p><span v-for='(item,index) in item.artists'>{{item.name}}&nbsp;
                            </span><span>-&nbsp;{{item.album.name}}</span></p>
                    </div>
                    <div class='playicon'>
            ...
            <van-button round type="info" @click='print' class='more'>打开酷网Q查看更多</van-button>
        </div>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async getMusic(id) { // 获取歌曲详情信息
                ...
                // 歌曲评论
                await axios.get('/comment/music?id=' + id + '&limit=' + 1).then(res => {
                    if (res.data.code === 200) {
                ...
                await axios.get('/lyric?id=' + id).then(res => {
                    if (res.data.code === 200) {
                        res.data.lrc.lyric.replace(/\[([^\]]+)\]([^\[]+)/g, (match, p1, p2) => {
                            let arr
                            arr = p1.split(':')
                            let formatTime = (Number(arr[0] * 60) + Number(arr[1])).toFixed(1)
                            this.lyric.push({
                                time: formatTime,
                                content: p2
                            })
            ...
            startLyric() {
                ...
                this.timer = setInterval(() => {
                    ...
                    for (let i = 0; i < this.lyric.length; i++) {
                        // 需要判断是否已经最后一句歌词
                        if (this.audio.currentTime >= this.lyric[this.lyric.length - 1].time) {
                            this.currentIndex = this.lyric[this.lyric.length - 1]
                            break // 终止循环
                        }
                        if (this.audio.currentTime > this.lyric[i].time && this.audio.currentTime < this.lyric[i + 1].time) {
                            this.currentIndex = i
...
<style>
    ...
    .bottom {
        background: url(../assets/d7e4e3a244701ee85fecb5d4f6b5bd57.png) no-repeat center center;
        background-size: cover;
        height: 500px;
        width: 500px;
        margin: 0 auto;
